...
def _api_key():
    """Resolve the Google API key: env var, then ~/.dusty/secrets.toml.

    The environment wins so a one-off export or CI still overrides the file.
    """
    ...
    try:
        ...
        return load(required=False).get("google", {}).get("api_key") or None
    except Exception:
        return None
...
def get_client():
    ...
    if not api_key:
        print(
            "Error: no Google API key. Set GOOGLE_API_KEY, or add\n"
            "  [google]\n  api_key = \"...\"\n"
            "to ~/.dusty/secrets.toml (run `dusty init` to create it)."
        )
        return None
    try:

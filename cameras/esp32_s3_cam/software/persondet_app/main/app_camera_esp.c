...
int app_camera_init() {
  ...
  camera_config_t config = {};   /* zero-init: newer esp32-camera fields
                                    (conv_mode, grab_mode, sccb_i2c_port)
  ...
  // GC0308 warm-up: first frames before AGC/AWB settle are grainy garbage;
  // capture and discard a few so real frames start clean.
  for (int i = 0; i < 5; i++) {
